...
Improvements implemented here:
  - Improvement 2:  Model loaded once at startup via load(), not per-request
  - Improvement 3:  ChromaDB persisted to disk; loaded from disk on subsequent starts
  - Improvement 6:  chunk_size=6000 prevents description splitting
  - Improvement 7:  ISBN stored as ChromaDB metadata (not embedded in text)
  - Improvement 8:  Emotion scores use MEAN aggregation (fixed in preprocessing script)
  - Improvement 9:  Semantic ranking preserved after .isin() lookup
  - Improvement 10: Oversample initial results when category/tone filters are active
  - Improvement 11: disgust and neutral columns dropped at load time
"""
